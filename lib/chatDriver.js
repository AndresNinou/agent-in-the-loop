...
async function configureAutoApproveSettings(frame, selectors) {
  ...
  try {
    ...
    // Close the settings panel by clicking the close button
    try {
      const closeButton = await driver.findElements(By.css(selectors.settingsCloseButton));
      if (closeButton.length > 0) {
        console.log('❌ Clicking close button to exit settings panel...');
        await closeButton[0].click();
        await new Promise(resolve => setTimeout(resolve, 500)); // Wait for panel to close
      }
    } catch (e) {
      console.log('⚠️ Could not find or click close button:', e.message);
    }

    console.log('✅ Auto-approve settings configured and panel closed successfully');
  } catch (e) {
...
async function sendMessage(frame, cfg) {
  ...
  // Filter out user messages and get only new assistant responses
  const newMessages = allMessagesAfter.slice(messagesBefore.length).filter(msg => {
    const msgLower = msg.toLowerCase().trim();
    const msgOriginal = msg.trim();
    
    // Filter out the user message we just sent
    if (msgLower === message.toLowerCase().trim()) {
      return false;
    }
    
    // Filter out messages that contain the user's exact message
    if (msgOriginal === message.trim()) {
      return false;
    }
    
    // Filter out user-like patterns and questions
    if (msgLower.startsWith('you:') || 
        msgLower.startsWith('user:') ||
        msgLower.endsWith('?') && msgLower.length < 100) { // Short questions are likely user messages
      return false;
    }
    
    // Filter out command-like messages (likely tool calls or system messages)
    if (msgOriginal.startsWith('python ') || 
        msgOriginal.startsWith('node ') || 
        msgOriginal.startsWith('npm ') ||
        msgOriginal.length < 15) { // Very short messages
      return false;
    }
  ...
  console.log(`📊 New assistant messages: ${newMessages.length}`);
